...
export class RythymScene extends Scene {
    ...
    _track = this.tracks[SaveData.nextSong];
    constructor() {
        super();
        this.bgPaletteIndex = 2;
        //
        this.addGameobject(new SpriteObject(96, 0, 0, 0, "bg_dots"));
        this._creatureObj = this.addGameobject(new AnimatedSpriteObject(32, 80, 0, 0, SaveData.getCreatureSprite(), creature, true));
        ...
        this._cauldronObj = this.addGameobject(new AnimatedSpriteObject(0, 80, 0, 0, "cauldron", cauldron, true));
        ...
        this._songNameDisplay = this._songNameDisplayFrame.addChild(new TextObject("The SpooKy Squash", "font_rabbit", 0, 8));
        this._songNameDisplay.localX = 80 - this._songNameDisplay.textRenderer.width / 2;
        this._noteStatusDisplay = this.addGameobject(new TextObject("", "font_rabbit", 48, 120, 3));
        ...
        this._transition = this.addGameobject(new SpriteObject(0, 0, 0, 0, "transition"));
    ...
    update(delta) {
        switch (this._state) {
            case RhythmState.READY:
                this.ReadyState(delta);
                break;
            case RhythmState.PAUSED:
                this.PausedState(delta);
                break;
            case RhythmState.PLAYING:
                this.PlayingState(delta);
                break;
            case RhythmState.ENDING:
                this.EndingState(delta);
                break;
        }
    }
    ReadyState(delta) {
    ...
    PlayingState(delta) {
        if (this._bigTextTime >= 0) {
            ...
            if (this._bigTextTime <= 0.5)
                this._noteStatusDisplay.localY = 124;
            if (this._bigTextTime <= 0)
                this._noteStatusDisplay.textRenderer.setText("");
        }
        if (this._track.notes.length == 0)
            return;
        if (Game.i.input.consumeInput(InputAction.START)) {
    ...
    lateDraw(gfx) {
        if (this._state == RhythmState.PAUSED)
            gfx.drawString("PAUSED", Game.i.assets.getSprite("font_rabbit"), 0, 0);
        for (let i = 0; i < this._track.notes.length; i++) {
            let y = Math.round((this._musicTrack.currentTime - this._track.notes[i].time) * 128 + 72);
            gfx.drawImage(this._rhythmSprite, this._track.notes[i].note * 16, 0, 16, 16, // src
            40, y, 16, 16 // dst
            );
        }
    }
...
(function (RhythmState) {
    RhythmState[RhythmState["READY"] = 0] = "READY";
    RhythmState[RhythmState["PLAYING"] = 1] = "PLAYING";
    RhythmState[RhythmState["PAUSED"] = 2] = "PAUSED";
    RhythmState[RhythmState["ENDING"] = 3] = "ENDING";
