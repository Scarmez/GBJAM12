...
export class RythymScene extends Scene {
    ...
    constructor() {
        ...
        this._creatureObj = this.addGameobject(new AnimatedSpriteObject(32, 80, 0, 0, "c1_infant", creature, true));
        ...
        this._songNameDisplay = this.addGameobject(new TextObject("The SpooKy Squash", "font_rabbit", 0, 4));
        this._songNameDisplay.localX = 80 - this._songNameDisplay.textRenderer.width / 2;
        this._noteStatusDisplay = this.addGameobject(new TextObject("", "font_rabbit", 100, 64));
    ...
    PlayingState(delta) {
        if (this._bigTextTime >= 0) {
            ...
            if (this._bigTextTime <= 0)
                this._noteStatusDisplay.localY = 64;
        }
    ...
    lateDraw(gfx) {
        ...
        for (let i = 0; i < track.notes.length; i++) {
            ...
            if (y > -16) {
                gfx.drawImage(this._rhythmSprite, track.notes[i].note * 16, 0, 16, 16, // src
