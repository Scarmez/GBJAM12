...
export class ChooseEggScene extends Scene {
    ...
    constructor() {
        ...
        this.eggChoices.push(this.addGameobject(new AnimatedSpriteObject(0, 0, 0, 0, "c1_egg", egg_anim, true)));
        this.eggChoices.push(this.addGameobject(new AnimatedSpriteObject(0, 0, 0, 0, "c2_egg", egg_anim, true)));
        this.choiceCursor = this.addGameobject(new SpriteObject(0, 0, 0, 0, "rhythm_inputs"));
        for (let i = 0; i < this.eggChoices.length; i++) {
            if (i > 0)
                this.eggChoices[i].animationController.paused = true;
        ...
        this.changeChoice(0);
        this.emote = this.addGameobject(new AnimatedSpriteObject(0, 0, 0, 0, "emotes", emotes_anim, true));
    ...
    changeChoice(input) {
        ...
        if (newIndex < 0)
            newIndex = 0;
        if (newIndex >= this.eggChoices.length)
            newIndex = this.eggChoices.length - 1;
        if (newIndex == this.currentChoice)
            return;
...
var Phase;
(function (Phase) {
    Phase[Phase["CHOOSE"] = 0] = "CHOOSE";
    Phase[Phase["HATCH"] = 1] = "HATCH";
})(Phase || (Phase = {}));
